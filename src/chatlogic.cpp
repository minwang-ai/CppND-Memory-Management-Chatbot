...
ChatLogic::ChatLogic()
{
    ...
    // no longer create instance of chatbot here as ChatLogic does not own it
    _chatBot = nullptr; // Initialize to nullptr
...
ChatLogic::~ChatLogic()
{
    ...
    // no longer delete chatbot instance here as ChatLogic does not own it
    // delete _chatBot;
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            childNode->get()->AddEdgeToParentEdge(edge.get());
                            parentNode->get()->AddEdgeToChildEdge(std::move(edge));
                        }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {   
                rootNode = it->get(); // assign current node to root (it needs to be raw pointer as it will be passed to function SetRootNode)
            }
            else
            {
